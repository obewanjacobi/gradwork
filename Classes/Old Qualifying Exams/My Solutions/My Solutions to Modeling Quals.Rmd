...
Consider when $\dot x = 0$. This implies that either $x = 0$ or $y = \frac{(1+x)(6-x)}{3} = -\frac{1}{3}x^2 + \frac{5}{3}x +2$. If we were to plot this, we would get a porabola. 

Then if $\dot y = 0$, this implies that $y = 0$ or $x = 2$. If we plot either of these, we get straight lines. Now, if we plot all of the lines mentioned, and do some phase plane analysis, we see that we have an area where the phase portrait always points inward. But we also have the point $(2,4)$ which is an unstable focus. Thus by Poincare-Bendixon, we know that we must have a limit cycle in the first quadrant.
