...
However, I am not sure if this is enough to show that every nonconstant positive solution is periodic, so to do that we will use Lyapunov functions. First off notice that $\frac{dy}{dx} = \frac{(2x-1)y}{x(1-y)}$, which is seperable. If we separate the variables and integrate accordingly, we get
$$\ln(y) - y = 2x - \ln(x) +C$$
...
First after some simplet algebra, we find that $E_1 = (0,0), E_2 = (4,0)$, and $E_3 = (1,3)$. So by linearizing the system, we find that
