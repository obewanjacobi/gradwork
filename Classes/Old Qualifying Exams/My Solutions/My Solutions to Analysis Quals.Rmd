...
(b)
Consider an example in $\mathbb{R}^2$ with the standard metric. Take $K$ as the $x$-axis and $F$ as the graph of the expnential function $\mathrm{e}^x$, that is, $F = \{(x,y) \in \mathbb{R}^2 | y = \mathrm{e}^x\}$. These are clearly non-empty and mutually disjoint. Both $K$ and $F$ are closed, but neither is compace because they are both unbounded. It is easy then to see that their distance is zero and a stricly positive $\delta$ of the desired result does not exist. Thus we can see then that if $K$ is not compact and still closed that our proof in part (a) may not hold.
