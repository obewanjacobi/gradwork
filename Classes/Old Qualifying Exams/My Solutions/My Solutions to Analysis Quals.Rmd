...
### If $f \in L^p([0,\infty))$ for $p \geq 1$, then $\lim_{n \to \infty} \int_0 ^{\infty} f(x) \mathrm{e}^{-nx} dx = 0$.
#### My Solution:
...
since $f \in L^p$. This argument works for any $1 \leq p < \infty$. As for $p = \infty$, this argument still works, however rather than saying  the above, we argue $\int f(x) \mathrm{e}^{-nx} \chi_B dx \leq \mathrm{ess}\sup{f} \chi_B < \infty$. So for any $p$ such that $1 \leq p \leq \infty$, we can see that the integral is indeed bounded. Thus by the Lebesgue Dominated Convergence Theorem,
$$\lim_{n \to \infty} \int^{\infty} _0 f(x) \mathrm{e}^{-nx} dx = \int^{\infty} _0 \lim_{n \to \infty} f(x) \mathrm{e}^{-nx} dx = \int 0 dx = 0$$ QED



### There is a sequence of polynomials $p_n$ such that $p_n(x) \rightarrow 1$ for all $x \in [0,1]$ and $\int_0 ^1 p_n \rightarrow 2$.
#### My Solution:
This problem relies on the Stone-Weierstrass Theorem, stating:
...
$f_n(x):=-4n^2x+1+4n \text{ for }x\in \left[\frac{1}{2n},\frac{1}{n}\right]\text{and }f(x)=1\text{ otherwise}.$ This gives us a function that follows this property, then using Stone's Theorem we know that there must be a polynomial that uniformly converges to our defined $f_n$'s. Thus the polynomial exists. QED



### Suppose that $f$ is absolutely continuous on $[0,1]$ and that $f(x) \neq 0$ for all $x \in [0,1]$. Prove that $1/f$ is absolutely continuous on $[0,1]$.
#### My Solution:
...
### Let $\{f_n\}$ be a sequence of Lebesgue measurable functions defined on $[0,1]$ with the property that $\{f_n(x)\}$ converges almost everywhere to a function $f$. Prove that $\lim_{n \to \infty} \int_{[0,1]} \sin(f_n(x)) dx = \int_{[0,1]} \sin(f(x)) dx$.
#### My Solution:
First note that we know from undergraduate analysis that if $f_n \to f$ pointwise almost everywhere, then the composition of $f_n$ must also converge to the composition of $f$. Thus, for any continuous function $g$, $g \circ f_n \to g \circ f$. Thus $\sin(f_n(x)) \to \sin(f(x))$ pointwise almost everywhere. Thus because of this and because for all $n \in \mathbb{N}$ $\sin(f_n(x)) \leq 1$, we can use the dominated convergence theorem here to show that
$$\lim_{n \to \infty} \int_{[0,1]} \sin(f_n(x)) dx = \int_{[0,1]} \sin(f(x)) dx$$ QED



###Is the closed unit ball of $L^2([0,1])$ closed in $L^1([0,1])$? Justify.
#### My Solution:
We can generalize this to $L^2(X)$ where $X$ is any measure space. Since we are in the context of metric spaces, we only need to check sequential closeness. Fix $\{f_n\}$ a sequence in the closed unit ball of $L^2(X)$ which converges to $f$ in $L^1$. Then $\int_X |f_n|^2 \leq 1$ for all $n$. Without loss of generality, we can assume that $f_n$ converges to $f$ pointwise. Now by Fatou's Lemma,
$$\int_X |f|^2 \leq \liminf \int_X |f_n|^2 \leq 1$$. Thus $L^2([0,1])$ is indeed closed in $L^1([0,1])$. QED



###Let $\{f_n\}$ be a sequence of real-values Lebesgue measurable functions defined on $\mathbb{R}$. Define $E=\{x \in \mathbb{R}| \lim_{n \to \infty} f_n(x)\mathrm{ exists}\}$. Prove that $E$ is Lebesgue measurable.
#### My Solution:
Both $\limsup f_n$ and $\liminf f_n$ exist for all $x \in X$ and are measurable functions. The set $E$ can also be written as

$$E = \left\{x \in X : \lim f_n(x) \text{ exists}\right\} = \left\{ x \in X : \limsup f_n(x) = \liminf f_n(x) \right\}$$

Define $h(x)$ such that 

$$h(x) = \begin{cases} 0 & \text{ if } \limsup f_n(x) = \liminf f_n(x) = \pm\infty\\
\limsup f_n(x) - \liminf f_n(x) & \text{ otherwise }
\end{cases}$$

$h$ is a measurable function because $\{x: \limsup f_n > \liminf f_n\}$ and $\{x: \limsup f_n < \liminf f_n\}$ are both measurable. Thus the set $E$ can be written as $E = h^{-1} (\{0\})$, implying that $E$ is indeed measurable. QED



### Let $\{f_n\}$ be a sequence of continuous functions defined on $\mathbb{R}$. Suppose that for every $x \in \mathbb{R}$ there exists $n \in \mathbb{N}$ such that $f_n(x) = 0$. Prove that there exists an open interval $I \in \mathbb{R}$ and an index $n \in \mathbb{N}$ such that $f_n(x) = 0$ for all $x \in I$.
#### My Solution:
First we will prove this with polynomials, then we will expand it with a density argument to prove it to be true for any continuous function. 

Suppose there does not exist an $n$ such that $p_n = 0$ for all $n \in \mathbb{N}$. Let's define $S_n = \{x \in I|p_n(x) = 0\}$. Note, $S_n$ has to be finite since each polynomial can only have a finite number of zeros. Now consider $\bigcup^{\infty} _{n=1} S_n \supseteq I$ since for each $x \in I$ there exists a polynomial such that $p_n(x) = 0$. But a countable collection of finite sets is countable. But $I$ is uncountable, thus giving us a contradiction. This implies that one of the polynomials must indeed be identically zero.

Now, using Stone's Theorem, we know that any polynomial can be approximated by a continuous function. So for all polynomials $p_n$, since the set of polynomials is dense in the set of continuous functions, it is easy to see that this applies to a sequence of continuous functions as well. QED



### Suppose $K$ is a compact metric space and $f_n:K \rightarrow \mathbb{R}$ is a sequence of continuous functions converging pointwise to a continuous function $f$ on $K$. Prove that if $\lim_{n \to \infty} f_n(x) = f(x)$ for every sequence $x_n$ converging to a point $x$, then $f_n$ converges uniformly to $f$.
#### My Solution:
First recall the definition of uniformly convergent. $f_n$ converges uniformly to $f$ on $K$ if for all $\epsilon > 0$ there exists an $N$ such that $|f_n(x) - f(x)| < \epsilon$ for all $x \in K$ for all $n \leq N$. 

So, we know then for all $\epsilon^*$, there exists a $\delta$ such that $|x_n -x| < \delta$ such that $|f_n(x) - f(x)| < \epsilon^*$. We can use this to show
$$|f_n(x) - f(x)| = |f_n(x) + f_n(x_n) - f_n(x_n) - f(x)| \leq |f_n(x) - f_n(x_n)|+|f_n(x_n)-f(x)|$$
$$< |f_n(x) - f_n(x_n)| + \epsilon^* = |f_n(x-x_n)| + \epsilon^* \leq |f(\delta)| + \epsilon^* < 2\epsilon^*$$

So if we let $\epsilon^* = \frac{\epsilon}{2}$, then we've shown that $f_n$ does indeed converge uniformly to $f$ on $K$. QED



### Let $f:[a,b] \rightarrow \mathbb{R}$ be a bounded function which is Riemann integrable on $[a,c]$ for every $c \in (a,b)$. Prove that $f$ is Riemann integrable on all of $[a,b]$.
#### My Solution:
Let $A = \{x|\lim_{x \to a} f(x) \neq f(a)\}$, so $A$ is the set of discontinuities of $f$ since $f$ is bounded and Riemann Integrable on $[a,c]$ for all $c \in (a,b)$. This implies that the number of discontinuties in $[a,c]$ must sum up to have measure zero. There is also one more point that could possibly be discontinuous, that being $b$. So if we let $\{d_i\}$ be the discontinuities on $[a,c]$ that sum up to measure zero, then $A = \{x|x \in \{d_i\}, x=b\}$. We know $m(b) = 0$ as it is a point and the measure of all the $d_i$'s is zero as well, thus $m(A) = 0$. Also $f$ is clearly measurable as $f$ is bounded and Riemann Integrable. Hence $f$ is Lebesgue measurable and $\int f = Rf$ on all of $[a,b]$. QED
...
$$\sup_{||f||_1 \leq 1} Tf = \sup_{||f||_1 \leq 1} \int fh \leq \sup_{||f||_1 \leq 1} ||f||_1 ||h||_{\infty}$$ Note, this is by Holder's since we have $1, \infty$, and
$$\leq 1 \cdot ||h||_{\infty} = ||h||_{\infty}$$

- Will show $\geq$:

We may assume $||h||_{\infty} = M > 0$. $\mathbb{R}$ is $\sigma$-finite, so there exists $F_n$ increasing towards $x$ such that $m(F_n) < a$. Define $A_n = \{x \in F_n| |h(x)| > a\}$ for $0<a<M$, to be fixed. Hence $m(A_n) > 0$. Define $g_n(x) = \frac{\mathrm{sgn}(h) - \chi_{A_n}}{m(A_n)}$, which implies $||g_n||_1 = 1$ for all $n$ and $\int h g_n = a$ for all $n$. Thus $a < \int h g_n$ for all $n$. This implies $\sup_{0<a<M} a < \sup_{||g_n||_1 \leq 1} \int h g_n$. $$||f||_{\infty} = M < \sup_{||f||_1 \leq 1} \int fh = \sup_{||f||_1 \leq 1} Tf$$
...
Since $f$ is integrable, we know that $\int_0 ^1 \sqrt[n]{f} \cdot \chi_{f>1} < \int_0 ^1 f < \infty$, and $\int_0 ^1 \sqrt[n]{f} \cdot \chi_{0<f<1}$ must be bounded, thus $\sqrt[n]{f}$ is integrable as well. So,
...
*Proof*: Let $t \in [0,m = ||f||_{\infty})$. Then $\int |f| = \int _{\{|f| < t\}} |f| + \int _{\{|f| \geq t \}} |f|$. Then $\int |f|^p \geq \int_{\{|f| < 1\}} |f|^p \geq \int_{\{|t| < 1\}} t^p$. Hence $(\int|f|^p)^{(1/p)} \geq (\int_{\{|f| <t\}} t^p)^{1/p} = t \cdot m(\{|f| < t\})^{1/p}$. This implies that $\liminf ||f||_p \geq \liminf t \cdot m(\{|f| < t\})^{1/p} = t \cdot 1$. Thus $\liminf ||f||_p \geq t$ for all $t \in [0, ||f||_{\infty})$. It follows that by our claims then that $\lim \sup||f||_p \leq ||f||_{\infty} \leq \liminf ||f||_p$. This implies that $\lim_{p \to \infty} ||f||_p = ||f||_{\infty}$ Thus we have proven the desired result. QED
...
by the M.C.T. since $\sum^k _{i=1} |f_n|$ is increasing. Thus $|f_n| \rightarrow 0$ almost everywhere because it's integrable when $n$ approaches $\infty$. Hence $f_n \rightarrow 0$ almost everywhere. QED 
...
$$\frac{d}{dt} \left( \int_a ^b f(x,t) dx \right) = \int^b _a \frac{\partial}{\partial t} f(x,t) dx$$
Thus we find that 
$$\frac{d}{dt} \int_0 ^1 \frac{\sin(xt)}{x}dx = \int_0 ^1 x \cdot \frac{\cos(xt)}{x} dx = \int_0 ^1 \cos(xt) dx = \frac{\sin(t)}{t}$$
where $t > 0$. QED
...
#### My Solution:
Let $A = \{ \chi_{(0,t)} : t \in (0,1)\}$. Then we claim for any $\chi_{(0,t_1)}, \chi_{(0,t_2)} \in A$ such that $t_1 \neq t_2$, so,
$$||\chi_{(0,t_1)} - \chi_{(0,t_2)}||_{\infty} = 1$$
To prove this, without loss of generality, let $t_1 < t_2$. Then $|\chi_{(0,t_1)} - \chi_{(0,t_2)}| = 0,1,0$ on $(0,t_1),(t_1,t_2),(t_2,1)$ respectively. This is where we get the above equality, as 
$$\left\|\chi_{(0,t_2)} - \chi_{(0,t_1)}\right\|_\infty = \sup_{x\in[0,1]}\left|\chi_{(0,t_2)}(x) - \chi_{(0,t_1)}(x)\right| = 1$$ Thus we have proven our claim. 

Now consider $\left\{ B_{\left(\chi_{(0,t)}, \frac{1}{3} \right)} \right\}$ which is an uncountable collection of disjoint balls. And given any dense set $S \subset L^{\infty}([0,1])$ has elements in each ball by definition of density. Thus $S$ is uncountable. QED
...
Consider $m(\{x | 0 < f<1\})$, which we claim is equal to zero. To prove this, pick $0< |f| \leq \epsilon < 1$. Then 
...
#### My Solution:
Suppose not! Then for all $I$, $m(E \cap I) \leq \frac{9}{10} m(I)$ and $m(E^c \cap I) \leq \frac{9}{10} m(I)$. Suppose $E$ has finite measure and let $E \subset \bigcup^{\infty}_{n=1} I_n$. Then
$$m(E) = m \left(E \cap \bigcup^{\infty}_{n=1} I_n \right) = m\left(\bigcup^{\infty}_{n=1} (E \cap I_n) \right) \leq \sum^{\infty}_{n=1} m(E \cap I_n) \leq \frac{9}{10} \sum^{\infty} _{n =1} m (I_n)$$
Thus for all covers of $E$, $m(E) \leq \frac{9}{10} \sum^{\infty} _{n =1} m(I_n)$. But, by definition 
$$m(E) = \inf \{\sum_n m(I_n) : E \subset \bigcup^{\infty}_{n=1} I_n\}$$
Hence 
$$m(E) \leq \frac{9}{10} m(E) \implies m(E) = 0$$
Now for $E$ with any measure, 
$$m(E \cap(-n,n) \cap I_n) \leq m(E \cap I_n) \leq \frac{9}{n}m(I_n)$$
...
Now assume $X$ is $\sigma$-finite. We have $X = \bigcup^{\infty}_{i = 1} A_n$, a union of disjoint sets with $\mu (A_n < \infty)$. Define $f = \sum^{\infty} _{n=1} a_n$ where $a_n = \frac{1}{\mu (A_n) \cdot n^2} \cdot \chi_{A_n}$ when $\mu(A_n) > 0$ and $a_n = \frac{1}{n^2} \chi_{A_n}$ when $\mu(A_n) = 0$. Then
...
#### My Solution:
(a)
By way of contradiction, assume for every $n$ we can find an $x_n \in K$ and $y_n \in F$ such that $\rho (x_n, y_n) < \frac{1}{n}$. Since $K$ is compact, there is a convergent subsequence $x_{n_k}$ whose limit is $x$. By the triangle inequality, $\rho(x,y_{n_k}) \leq \rho(x,x_{n_k}) + \rho(x_{n_k},y_{n_k})$. If we juggle the $\epsilon$'s around a bit, we find that $x \in F$ giving us a contradiction, thus we have proven the desired result. The problem here is that this question is not written that well in regards to our qualifying exam, so for the most part this one should be ignored. It leaves out important details of what exactly $\rho$ is. However, if you would like to check out other solutions on this, follow these links:
...
(b)
Consider an example in $\mathbb{R}^2$ with the standard metric. Take $K$ as the $x$-axis and $F$ as the graph of the exponential function $\mathrm{e}^x$, that is, $F = \{(x,y) \in \mathbb{R}^2 | y = \mathrm{e}^x\}$. These are clearly non-empty and mutually disjoint. Both $K$ and $F$ are closed, but neither is compact because they are both unbounded. It is easy then to see that their distance is zero and a strictly positive $\delta$ of the desired result does not exist. Thus we can see then that if $K$ is not compact and still closed that our proof in part (a) may not hold.
...
#### My Solution:
Because $f(-x) = -f(x)$, it is sufficient to show this is true for $(0,1)$ as $(-1,0)$ follows from it. Well,
$$T.V.(f) = \int_0 ^1 |f'(x)| dx = \int_0 ^1 \frac{\left|\cos\left(\frac{1}{x}\right) - 2x \sin\left(\frac{1}{x}\right) \right| \ln(x)}{\frac{1}{x^2}} dx$$
Then if we let $u = \frac{1}{x}$ and $du = \ln(x) dx$, then 
$$T.V.(f) = \int_1 ^{\infty} \frac{\left|\cos(u) - \frac{2}{u} \sin(u)\right|}{u^2} du \leq \int_1 ^{\infty} \frac{du}{u^2} = 1 < \infty$$
...
Well, let 
$$P = \{-1\} \cup \left[ \{ \pm \sqrt{\frac{1}{n \pi}} : n \in \mathbb{N} \} \cap [-1,1] \right] \cup \{1\}$$. Hence for $x_n, x_{n+1} \in P$,
$$\sum^{\infty} _{n=1} |f(x_n) - f(x_{n+1})| = \sum^{\infty} _{n=1} \left| f \left(\sqrt{\frac{1}{n \pi}} \right) - f \left( \sqrt{\frac{1}{(n+1) \pi}} \right) \right|$$
...
### Let $\mu^*$ be an outer measure on a set $X$. Prove that if $\{E_k\}$ is a sequence of subsets of $X$ and $\sum^{\infty} _{k=1} \mu ^* (E_k) < \infty$, then $\mu^*(\bigcap^{\infty} _{k = 1} \bigcup^{\infty} _{n=k} E_n) = 0$
#### My Solution:
Since $\sum^{\infty} _{k=1} \mu^* (E_k) < \infty$, for any $\epsilon > 0$ there exists an $N$ such that $\sum^{\infty} _{k=N} \mu^* (E_k) < \epsilon$ from sub-additivity. Thus
$$\mu^*\left( \bigcup^{\infty} _{n=N} E_n \right) \leq \sum^{\infty} _{k=N} \mu^* (E_k) < \epsilon$$
But by monotonicity, 
$$\mu^* \left( \bigcap^{\infty} _{k=1} \bigcup^{\infty} _{n=k} E_n \right) \leq \mu^* \left( \bigcup^{\infty} _{n=N} E_n \right) < \epsilon$$
Since $\epsilon$ is arbitrary, we see that $\mu^* \left( \bigcap^{\infty} _{k=1} \bigcup^{\infty} _{n=k} E_n \right) = 0$. QED



### Let $(X, \mathcal{A}, \mu)$ be a measure space and let $f:X \rightarrow (0,\infty)$ be measurable. Prove that $1/f$ is measurable.
#### My Solution:
...
### Let $\mu_n$ and $\mu$ be finite measures on a measurable space $(X, \mathcal{A})$ with the property that $\mu_n(A) \rightarrow \mu(A)$ for all $A \in \mathcal{A}$. Prove that $\int_X f d\mu_n \to \int_X f d\mu$ for every bounded measurable function $f:X \rightarrow \mathbb{R}$.
#### My Solution:
Let $f$ be a simple non-negative measurable function. Then 
$$\lim_{n \to \infty} \int_X f d\mu_n = \lim_{n \to \infty} \int_X \sum^L _{i=1} a_i \chi_{A_i}(x) d \mu_n = \lim_{n \to \infty} \sum^L _{i=1} a_i \mu_n(A_i) = \sum^L _{i=1} a_i \mu(A_i) = \int_X f d\mu$$ QED



### Suppose that $f \in L^1(\mathbb{R})$ with Lebesgue measure. Prove that $\lim_{n \to \infty} \frac{1}{2n} \int_{-n} ^n fdm = 0$.
#### My Solution:
We note that $\left| \frac{1}{2n} f(x) \chi_{\{|x| \leq n\}}(x) \right| \leq |f(x)|$ for $x \in \mathbb{R}$. Since $f \in L^1$, we may apply the DCT with $|f(x)|$ as the majorant. Thus
$$\lim_{n \to \infty} \int_{\mathbb{R}} \frac{1}{2n} f(x) \chi_{\{|x| \leq n\}}dm = \int_{\mathbb{R}} \lim_{n \to \infty} \frac{f(x)}{2n} \chi_{\{|x| \leq n\}}dm = \int_{\mathbb{R}} 0dm = 0$$. QED



### Let $\mu$ and $\nu$ be finite (positive) measures on a measurable space $(X, \mathcal{A})$. Define $\rho = \mu - \nu$. Prove that $|\rho|(E) \leq \mu(E) + \nu(E)$ for every set $E \in \mathcal{A}$.
#### My Solution:
Let $P_{\rho}$ and $N_{\rho}$ be a Hahn decomposition with respect to $\rho$. So
$$|\rho|(A) = \rho(A \cap P_{\rho}) - \rho(A \cap N_{\rho}) = \mu(A \cap P_{\rho}) - \nu(A \cap P_{\rho}) - \mu(A \cap N_{\rho}) + \nu(A \cap N_{\rho})$$
...
### Let $V$ be a normed vector space and let $L:V \rightarrow \mathbb{R}$ be linear. Define what is meant by $||L||$ and prove that $||L|| < \infty$ if and only if $L$ is continuous.
#### My Solution:
First off, $||L|| = \sup_{||V|| \leq 1} \{|L(V)|\}$.

Now we just need to prove that $||L||< \infty$ implies continuity, or that if it's not continuous that $||L|| = \infty$. Let $X \in V$ be such that there exists $\epsilon > 0$ such that in any $B(x,r_n)$ there exists $y_n$ such that 
$$|L(x)-L(y_n)| = |L(x-y_n)| > \epsilon$$
This implies that $||L|| \geq \frac{\epsilon}{r_n}$. Take $r_n' = \left( \frac{1}{2} \right)^n$. Clearly, $||L||$ is unbounded here. 
...
$$\lim_{v \to c} \frac{f(v) - f(c)}{v-c}$$ exists. The **derivative** of $f$ at $c$ is the value of the aforementioned limit denoted by $f'(c)$.
...
- Let $\{E_i\}_{i=1}^n$ be a finite disjoint collection of measurable subsets of a set of finite measure $E$. For $1 \leq i \leq n$, let $a_i$ be a real number. If $\phi = \sum_{i=1}^n a_i \times \chi_{E_i}$ on $E$, then 
...
- **Cauchy-Schwarz Inequality**: Let $E$ be a measurable set and $f$ and $g$ measurable functions on $E$ for which $f^2$ and $g^2$ are integrable over $E$. Then their product $f \cdot g$ is also integrable over $E$ and 
