...
$$f(x) = f(0) + \int_0 ^x g$$
This implies that $\int_0 ^x f' = \int_0 ^x g$, thus by the fundamental theorem of calculus, if we take the derivative of both sides, we can see that $f'(x) = g(x)$ for all $x$. QED
...
We repeat these definitions in this pattern, giving us that $\lim_{n \to \infty} m(E_n) = 0$. But for all $n \in \mathbb{N}$, $\bigcup^{\infty} _{k=n} E_k = [0,1]$ implying that $\lim \sup (E_n) = [0,1]$ giving us the desired result. QED
...
###If $f$ is nonnegative and integrable on $[0,1]$, then $\lim_{n \to \infty} \int_0 ^1 \sqrt[n]{f} = \lambda \{x| f(x) > 0\}$
#### My Solution:
...
$$||x \cdot f(x)||_1 \leq ||x||_2 ||f||_2$$ by Holders'.
Then,
...
#### My Solution:
The Cantor Function satidfies this problem.



### Let $E_1, E_2, E_3,...$ be a sequence of measurable subsets of $\mathbb{R}$ with the property that $\sum^{\infty} _{n=1} \lambda (E_n) < \infty$. Show almost every $x \in \mathbb{R}$ is contained in only finitely many of the $E_n$.
#### My Solution:
...
#### My Solution: 
Suppose $f_n \rightarrow f$ in $L^p$. Then $||f_n||_1 \rightarrow ||f||_1$. But $\int|f_n| = \frac{1}{n} \cdot n = 1$ for all $n$. Hence $||f|| = 1$. However, this is a contradiction because $\lim_{n \to \infty} f_n = 0$, thus it does not converge in $L^1 ([0,1])$
...
Now assume that $\{x: f(x) \ geq s\}$ is measurable for all $s \in S$. It suffices to show that $\{: f(x) > s\}$ is measurable for all $s \in S^c$. So, let $t \in S^c$, given that $S$ is dense in $\mathbb{R}$. Let $\{t_n\} \subset S$ such that $t_n$ decreases to $t$. Thus,
...
### Let $\lambda(G)$ denote the Lebesgue measure of the set $G$. Find an open set $G$ which is dense in $[0,1]$ such that $\lambda(G) <1$ and $\lambda(G \cap I) > 0$ for any interval $I \subset [0,1]$.
#### My Solution:
...
$$= \lim_{n \to \infty} ||f_n||_p \cdot ||g_n - g||_q + ||f_n -f||_p \cdot ||g||_q = 0$$
where $\lim_{n \to \infty} ||f_n||_p \rightarrow ||f||_p < \infty$, $||g_n-g||_q \rightarrow 0$, and $||f_n -f||_p \rightarrow 0$. Thus  we have proven the desired result. QED
...
by the M.C.T. since $\sum^k _{i=1} |f_n|$ is increasing. Thus $|f_n| \rightarrow 0$ almost everywhere when $n$ approachest $\infty$. Hence $f_n \rightarrow 0$ almost everywhere. QED 
...
Suppose $f$ is absolutely continuous on $[-1,1]$. Hence it is of bounded-variation on $[-1,1]$, thus $V_f [0,1] < \infty$. Consider the partition with enpoints 
$$P = \{-1\} \cup \left[ \{ \pm \sqrt{\frac{1}{n \pi}} : n \in \mathbb{N} \} \cap [-1,1] \right] \cup \{1\}$$
Hence for $x_i, x_{i+1} \in P$,
$$\sum^{\infty} _{i =1} |f(x_i) - f(x_{i+1})| = \sum^{\infty} _{i =1} \left|f \left(\sqrt{\frac{1}{n \pi}} \right) - f \left(\sqrt{\frac{1}{(n+1)\pi}} \right) \right|$$
$$= \sum^{\infty} _{i =1} \left| \left[ \left(\sqrt{\frac{1}{n \pi}} \right)^2 \cdot \cos \left( \frac{1}{\sqrt{\frac{1}{(n+1) \pi}}^2} \right) \right] - \left[ \left( \sqrt{\frac{1}{(n+1) \pi}}\right)^2 \cdot \cos \left( \frac{1}{\sqrt{\frac{1}{(n+1) \pi}}^2}\right) \right]  \right|$$
$$= \sum^{\infty} _{i =1} \left| \left( \frac{1}{n \pi} \cos(n \pi) \right) - \left( \frac{1}{(n+1) \pi} \cos((n+1) \pi) \right) \right| = \sum^{\infty} _{i =1} \left| \frac{1}{n \pi} - \frac{-1}{(n+1) \pi} \right|$$
because $n$ is even and $n+1$ is odd. The above sum is equal to $\frac{1}{\pi} \sum^{\infty} _{i =1} \left| \frac{2n+1}{n^2 +n} \right|$ which diverges because it's a harmonic series. Thus $V_f[0,1] \not< \infty$. This is a contradiction to the assumption of $f$ being absolutely continuous, thus we have proven the desired result. QED
...
### Suppose that $1< q,p < \infty$ and that $\frac{1}{p} + {1}{q} = 1$. Prove that if $f_n \rightarrow f$ in $L^p(\mathbb{R})$ and $g_n \rightarrow g$ in $L^q(\mathbb{R})$, then $f_ng_n \rightarrow fg$ in $L^1(\mathbb{R})$. 
#### My Solution:
...
where $\lim_{n \to \infty} ||f_n||_p \rightarrow ||f||_p < \infty$. Thus $||g_n - g||_q \rightarrow 0$ and $||f_n - f||_p \rightarrow 0$. So the above limit does in fact imply that $f_ng_n \rightarrow fg$ in $L^1(\mathbb{R})$. QED
...
$$\int|f|^p = \int_{\{x:|f|<1\}} |f|^p + \int_{\{x: |x| >1\}}|f|^p$$
For $p>0$, $\int_{\{x:|f(x)<1\}}|f|^p < \int_{\{x:|f(x)<1\}}|f|$. Thus
$$\int_{\{x:|f|<1\}} |f|^p + \int_{\{x: |x| >1\}}|f|^p \leq \int_{\{x:|f|<1\}} |f| + \int_{\{x: |x| >1\}} m^p$$
where $\int_{\{x:|f|<1\}} |f| < \infty$ and $\int_{\{x: |x| >1\}} m^p = m^p \cdot m(\{x:|f(x) > 1\}) < \infty$ thus the entire equation is less than $\infty$. So $(\int|f|^p)^{1/p} < \infty$ for all $p$. Thus $f \in L^p$. QED
...
### If $E$ is a measurable subset of $\mathbb{R}$, then there is an interval $I$ such that $\lambda(E \cap I) > \frac{9}{10} \lambda(I)$ or $\lambda(E^c \cap I) > \frac{9}{10} \lambda (I)$.
#### My Solution:
...
$$X = \{f> 0\} = \bigcup^{\infty}_{n =1} \{f > \frac{1}{n}\}$$
where 
$$\mu \left(\{f > \frac{1}{n}\} \right) \leq \int \frac{|f|}{\frac{1}{(1/n)}}d \mu \leq n \cdot \int|f| < \infty$$
