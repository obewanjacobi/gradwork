...
\[
\bea
\nnn 1-\alpha&=& P\left(\frac{\|\X(\hat{\bet}-\bet)\|^2}{(k+1)s^2} \sim F(k,n-k-1) \leq F_{\alpha,k,n-k-1}\right) \\
\nnn &=& P\left(\|\X(\hat{\bet}-\bet)\|^2 \sim (k+1)s^2 F(k+1,n-k-1) \leq F_{\alpha,k+1,n-k-1}\right).
\eea
\]
...
\nnn &=& (\hat{\beta}_0-\beta_0, \hat{\beta}_1-\beta_1) \bpm 4 & 1 \\ 1 & 3 \epm \bpm\hat{\beta}_0-\beta_0& \hat{\beta}_1-\beta_1\epm \\
...
\[
F(\bet,\sigma^2,\lam)=\ell(\bet,\sigma^2)+\lam\tr(\C\bet-\t)\tr.
\]
...
\eea
\]
Plugging $\sigma^2\hat{\lam}$ back into the expression for $\hat{\bet}_c$, we obtain 
\[
\hat{\bet}_c=\hat{\bet}-(\X\tr\X)^{-1}\C\tr(\C(\X\tr\X)^{-1}\C\tr)^{-1}(\C\hat{\bet}-\t).
\]
Differentiating $F$ with respect to $\bet$, we obtain
