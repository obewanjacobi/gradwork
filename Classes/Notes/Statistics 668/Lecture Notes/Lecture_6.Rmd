...
\[
Q(b_0,b_1)=\ep\tr\ep=\sum_{i=1}^n \varepsilon_i^2=\sum_{i=1}^n \left(y_i-b_0-b_1 x_i\right)^2.
\]
...
\]
where $\ds{s_{xy}=\sion (x_i-\bar{x})(y_i-\bar{y})=\sion (x_i-\bar{x})y_i=\sion x_i(y_i-\bar{y})}$ and $\ds{s_{x}^2=\sion (x_i-\bar{x})^2=\sion x_i^2-n\bar{x}^2}$.
...
\nnn s_{xy}&=& \hat{\beta}_1 s_{x}^2 \\
\nnn \hat{\beta}_1&=&\frac{s_{xy}}{s_{x}^2} \ \ \ \ (s_x^2>0 \mbox{ since not all } x_i\mbox{'s are the same}).
\eea
\]
- Note that 
\[
s_{xy}=\sion (x_i-\bar{x})(y_i-\bar{y})=\sion (x_i-\bar{x})y_i-\bar{y}\sion(x_i-\bar{x})=\sion (x_i-\bar{x})y_i-0,
\]
...
\eea
\]
- **Theorem 6.2.2**: For the simple linear regression model, we have $E(\hat{\beta}_0)=\beta_0$, $E(\hat{\beta}_1)=\beta_1$, $\ds{\var(\hat{\beta}_0)=\frac{\sigma^2}{s_x^2}}$ and $\ds{\var(\hat{\beta}_1)=\sigma^2\left(\frac{1}{n}+\frac{\bar{x}^2}{s_{x}^2}\right)}$
...
\[
\bea
\nnn \EV{\hat{\beta}_1}&=& \EV{\frac{\sion (x_i-\bar{x})y_i}{s_x^2}} \\
\nnn &=& \EV{\frac{\sion (x_i-\bar{x})}{s_x^2} y_i} \\
\nnn &=& \frac{\sion (x_i-\bar{x})}{s_x^2}\EV{y_i} \\
\nnn &=& \frac{\sion (x_i-\bar{x})}{s_x^2}(\beta_0+\beta_1 x_i) \\
\nnn &=& \frac{\beta_0}{s_x^2}\sion (x_i-\bar{x})+\frac{\beta_1}{s_x^2}\sion (x_i-\bar{x})x_i-0 \\
\nnn &=& 0+\frac{\beta_1}{s_x^2}\sion (x_i-\bar{x})x_i-\frac{\beta_1}{s_x^2}\sion (x_i-\bar{x})\bar{x} \\
\nnn &=& 0+\frac{\beta_1}{s_x^2}\sion (x_i-\bar{x})^2 \\
\nnn &=& \beta_1.
\eea
\]
...
\[
\bea
\nnn \cov(y_1,\hat{\beta}_1)&=& \cov(y_i,\sum_{j=1}^n \frac{(x_j-\bar{x})y_j}{s_x^2}) \\
\nnn &=& \sum_{j=1}^n\frac{(x_j-\bar{x})}{s_x^2}\cov(y_i,y_j) \\
\nnn &=& \frac{(x_i-\bar{x})}{s_x^2}\var(y_i) \\
\nnn &=& \frac{(x_i-\bar{x})}{s_x^2}\sigma^2 
\eea
\]
and 
\[
...
\nnn &=& \frac{1}{n}\sion \frac{(x_i-\bar{x})}{s_x^2}\sigma^2\\
\nnn &=& \frac{\sigma^2}{ns_x^2}\sion (x_i-\bar{x})\\
\nnn &=& 0.
\eea
\]
Now, we compute 
...
\nnn &=& \sion\left\{\left(1-\frac{1}{n}\right)\sigma^2+(x_i-\bar{x})^2\frac{\sigma^2}{s_x^2}-2\left(x_i-\bar{x}\right) \left(\left(1-\frac{1}{n}\right)\sigma^2-0\right)\right\} \\
\nnn &=& \sion\frac{n-1}{n}\sigma^2+\sion(x_i-\bar{x})^2\frac{\sigma^2}{s_x^2}-2\left(1-\frac{1}{n}\right)\sigma^2\sion\left(x_i-\bar{x}\right) \\
...
- Consider the following interesting decomposition of $\ds{s_y^2=\sion (y_i-\bar{y})^2}$:
\[
\bea
...
- Since $SST=\sion (y_i-\bar{y})^2=(n-1)s_y^2$ and $SSR=\sion(\hat{y}_i-\bar{y})^2=\hat{\beta}_1^2\sion(x_i-\bar{x})^2= \hat{\beta}_1^2s_{x}^2$, it follows that
\[
\bea
...
\eea
\]
- Note that $\ds{\frac{s_{x}}{\sqrt{n-1}}}$ is the sample standard deviation of the $x$'s and $\ds{\frac{s_{y}}{\sqrt{n-1}}}$ is the sample standard deviation of the $y$'s.
