...
\nnn &=& (2\pi)^{-p/2} e^{-(\y-\bmu)'(\Sig^{1/2})^{-1}\Sig^{-1/2}(\y-\bmu)/2} \left(\det\left(\Sig\right)\right)^{-1/2} \\
...
\nnn &=&(\y-\bmu-\Sig\t)'\Sig^{-1}(\y-\bmu\Sig\t)-\t\Sig\t-2\t'\bmu.
\eea
\]
Then we have
\[
\bea
\nnn M_{\y}(\t)&=& \int_{-\infty}^\infty \cdots \int_{-\infty}^\infty (2\pi)^{-p/2} |\Sig|^{-1/2} e^{(-1/2)\left\{(\y-\bmu-\Sig\t)'\Sig^{-1}(\y-\bmu\Sig\t)-\t\Sig\t-2\t'\bmu\right\}} d\y \\
\nnn &=& e^{\t'\bmu+\frac{1}{2}\t\Sig\t}\int_{-\infty}^\infty \cdots \int_{-\infty}^\infty (2\pi)^{-p/2} |\Sig|^{-1/2} e^{-(\y-\bmu-\Sig\t)'\Sig^{-1}(\y-\bmu\Sig\t)/2} d\y \\
\nnn &=& e^{\t'\bmu+\frac{1}{2}\t\Sig\t}.
\eea
\]
- **Theorem 4.3.3** (p.92):
...
\nnn &=& M_{\x}(\t_1)M_{\x}(\t_2)\exp\left\{\frac{1}{2}\left(\t_x'\Sig_{xy}\t_y+\t_y'\Sig_{yx}\t_x\right)\right\}.
\eea
\]
Since $\Sig_{xy}=\O$, we have $\Sig_{yx}=\Sig_{xy}'=\O'$ so that $M_{\v}(\t)=M_{\x}(\t_1)M_{\y}(\t_2)$ which implies that $\x$ and $\y$ are independent by Theorem 4.3.3(b).  
...
- *Answer*: Let $\y=\bpm y_1 \\ y_2 \\ y_3\epm$, $\A=\bpm 1 & 2 & 0 \\ 0 & 1 & -1\epm$, and $\B=\bpm 0 & 0 & 1 \epm$. Note that $\A\y=\bpm y_1 + 2y_3 \\ y_2-y_3 \epm$ and $\B\y=y_3$.  Then Theorem 4.4.4 implies that they are independent since
\[
\bea
\nnn \A\cov(\y)\B'&=&\bpm 1 & 2 & 0 \\ 0 & 1 & -1\epm \bpm 9 & 0 & -2 \\ 0 & 4 & 1\\ -2 & 1 & 1\epm \bpm 0 \\ 0 \\ 1 \epm \\
\nnn &=& \bpm 9 & 8 & 0 \\ 2 & 3 & 0 \epm \bpm 0 \\ 0 \\ 1 \epm \\
\nnn &=& \bpm 0 \\ 0 \epm .
\eea 
\]
