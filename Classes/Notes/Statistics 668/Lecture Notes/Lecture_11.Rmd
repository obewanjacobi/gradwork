...
\[
p(\bet,\tau)=c_1 \tau^{(\alpha_*+k+1)/2} \exp\left\{\tau[(\bet-\bphi)\tr\V^{-1}(\bet-\bphi)+\delta_*]/2\right\}, \tau>0
\]
...
\nnn &=& c_3 \left[(\bet-\bphi_*)\tr\V_{*}^{-1}(\bet-\bphi_*)+\delta_{**}\right]^{-(\alpha_{**}+k+1)/2+1} \\
...
t(a)%*%W.star%*%a
```
So, a 95% credible set for $\beta_2$ is $\a\tr\bphi_*\pm t_{\omega/2,n+2\alpha}(\a\tr\W_*\a)$
as we compute below.
```{r}
omega=.95
