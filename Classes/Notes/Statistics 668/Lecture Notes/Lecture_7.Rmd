...
There is another geometrical intrepretation if we look at the column of $\X$ and $\y$.
...
and 
\[
\frac{\partial \ell}{\partial \sigma^2}=-\frac{n}{2\sigma^2}+\frac{1}{2(\sigma^2)^2}Q(\hat{\bet}).
\]
...
\nnn &=&(\X\tr\X)^{-1}\X\tr\cov(\y)\X\tr(\X\tr\X)^{-1} \\
\nnn &=&(\X\tr\X)^{-1}\X\tr(\sigma^2\I)\X\tr(\X\tr\X)^{-1} \\
\nnn &=&\sigma^2(\X\tr\X)^{-1}\X\tr\X\tr(\X\tr\X)^{-1} \\
\nnn &=&\sigma^2(\X\tr\X)^{-1}.
\eea
\]
...
\[
s_{yj}=\frac{1}{n-1}\sion (y-\bar{y})(x_{ij}-\bar{x}_j),
\]
then 
\[
...
\[
\bea
\nnn Q_V(\b)&=& \ep\tr\V^{-1}\ep \\
\nnn &=& (\y-\X\b)\tr\V^{-1}(\y-\X\b) \\
...
- **Theorem 7.8.1** (p.165): Given observed data $(x_{i1},\ldots,x_{ik},y_i)$ for $i=1,\ldots, n$ from a multiple linear regression model where $n>k$ and $\X$ has rank $k+1$ (it is full rank), the least squares estimate of $\bet$ is 
\[
...
\[
E(s_1^2)=\sigma^2+\frac{\bet_2\tr\X_2\tr(\I-\X_1\tr(\X_1\tr\X_1)^{-1}\X_1\tr)\X_2\bet_2}{n-p-1}.
\]
- *Proof*: The result holds since Theorem 5.2.1 implies that
\[
\bea
\nnn E\left[(y-\X_1\hat{\bet}_1^*)\tr(y-\X_1\hat{\bet}_1^*)\right]&=& E\left[\y\tr\left(\I_n-\X_1(\X_1\tr\X_1)^{-1}\X_1\tr\right)\y\right]\\
...
- Here are the steps:
    + Step 1: Regress $\y$ on $\X_1$ to obtain $\hat{\bet}_1^*=(\X_1\tr\X)^{-1}\X_1\tr\y$ and residuals $\ep_1^*=\y-\hat{\y}_1$ where $\hat{\y}_1=\X_1\hat{\bet}_1^*$.
